...
Description : Root Terraform module. Provisions the full EnergyZero data
              ...
              (medallion), Key Vault (CMK), ADF, Databricks, and monitoring.
...
provider "azurerm" {
  features {
    ...
    databricks {
      # Prevent accidental workspace deletion with active clusters
    }
  }
}
...
# ---------------------------------------------------------------------------
# Microsoft Defender for Storage (two lines — huge security uplift)
# ---------------------------------------------------------------------------
